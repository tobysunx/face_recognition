...
class   RegisterUi(wx.Frame):
    ...
    def _open_cap(self,event):
        ...
        while self.cap.isOpened():
            ...
            self.imgtoshow = cv2.resize(self.im_rd, (480, 320), interpolation=cv2.INTER_CUBIC)
            self.im_rd = cv2.resize(self.im_rd, (120, 80), interpolation=cv2.INTER_CUBIC)
            # 人脸数 rects
            ...
            if self.second % 3 == 0:
                ...
                if len(self.rects) != 0:
                    ...
                    for k, d in enumerate(self.rects):
                        # 根据人脸大小生成空的图像
                        # 最后一个参数是线宽
                        cv2.rectangle(self.imgtoshow, tuple([d.left()*4, d.top()*4]), tuple([d.right()*4, d.bottom()*4]), (255, 0, 0), 2)

                    # 显示人脸数
                    self.OnShortCutButtonClicked(event)
               

            # print(im_rd.shape)
            height, width = self.imgtoshow.shape[:2]
            image1 = cv2.cvtColor(self.imgtoshow, cv2.COLOR_BGR2RGB)
